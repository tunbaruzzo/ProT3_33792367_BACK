<!--Registrate-->
<section class="registro-section" style="background-image: url('public/assets/img/fondo.registro.png'); background-size: cover; background-position: center; min-height: 100vh; display: flex; align-items: center; padding: 2rem;">
  <div class="container">
    <div class="row justify-content-center align-items-center">
        
      <!-- Columna izquierda -->
      <div class="col-md-6 d-flex flex-column align-items-center justify-content-center text-center">
        <img src="public/assets/img/robot.registrate.png" alt="Robot" class="img-fluid" style="max-height: 80%;" />
      </div>
      
      <!-- Columna derecha -->
      <div class="col-md-6 d-flex justify-content-center">
        <div class="form-container registro-form">
          <h2 class="mb-4 text-white">Registro</h2>
          <form method="post" action="<?= base_url('/formValidation') ?>">
            <div class="mb-3">
              <label for="nombre" class="form-label text-white">Nombre completo</label>
              <input type="text" class="form-control" name="nombre_completo" id="nombre" placeholder="Ingresa tu nombre" required />
            </div>
            <div class="mb-3">
              <label for="email" class="form-label text-white">Email</label>
              <input type="email" class="form-control" name="email" id="email" placeholder="ejemplo@correo.com" required />
            </div>
            <div class="mb-3">
              <label for="usuario" class="form-label text-white">Usuario</label>
              <input type="text" class="form-control" name="usuario" id="usuario" placeholder="Nombre de usuario" required />
            </div>
            <div class="mb-3">
              <label for="password" class="form-label text-white">Contraseña</label>
              <input type="password" class="form-control" name="password" id="password" placeholder="********" required />
            </div>
            <button type="submit" class="btn btn-cv w-100">Registrarse</button>
          </form>
        </div>
      </div>

    </div>
  </div>
</section>


<!--Fin de registrate-->


     